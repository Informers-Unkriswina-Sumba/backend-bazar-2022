...
function generateCustomerInformation(doc, invoice) {
    ...
    doc
        ...
        .text(formatCurrency(invoice.totalPesanan), 150, customerInformationTop + 75)
        .font('Helvetica-Bold')
        .text(invoice.pembeli.nama, 300, customerInformationTop)
        .font('Helvetica')
        .text(invoice.pembeli.type, 300, customerInformationTop + 15)
        .text('Waingapu, Nusa Tenggara Timur, Indonesia', 300, customerInformationTop + 30)
        .moveDown();
    generateHr(doc, 252);
}
function generateInvoiceTable(doc, invoice) {
    ...
    generateTableRow(doc, invoiceTableTop, 'Item', 'Harga Satuan', 'Jumlah', 'Total Harga');
    generateHr(doc, invoiceTableTop + 20);
    doc.font('Helvetica');
    for (i = 0; i < invoice.items.length; i++) {
        const item = invoice.items[i];
        const position = invoiceTableTop + (i + 1) * 30;
        generateTableRow(doc, position, item.item, formatCurrency(item.amount), item.qty, formatCurrency(item.amount * item.qty));
        generateHr(doc, position + 20);
    }
    const subtotalPosition = invoiceTableTop + (i + 1) * 30;
    generateTableRow(doc, subtotalPosition, '', 'Total Belanja', '', formatCurrency(invoice.totalPesanan));
    ...
    doc.font('Helvetica-Bold');
    generateTableRow(doc, duePosition, '', 'Total Tagihan', '', formatCurrency(invoice.totalPesanan));
    doc.font('Helvetica');
}
function generateFooterUpdatedAt(doc, invoice) {
    doc
        .fontSize(10)
...
function generateFooter(doc) {
    doc
        .fontSize(10)
        .text(`Invoice ini sah dan diproses oleh komputer. silakan hub nomor dibawah ini jika butuh bantuan: 082217971133`, 50, 750, { align: 'center', width: 500 });
}
